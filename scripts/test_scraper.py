...
def main():
    print("Starting Content Scraper Test Script")
    
    csv_path = os.path.join(os.path.dirname(current_dir), 'data', 'articles.csv')
    
    if os.path.exists(csv_path):
        print(f"Loading articles from {csv_path}...")
        df = pd.read_csv(csv_path)
        # Replace NaN with None to match DB return behavior for 'source'
        df = df.where(pd.notnull(df), None)
        articles = df.to_dict('records')
    else:
        print("CSV not found, fetching from DB...")
        # Changed limit to 1000 as requested
